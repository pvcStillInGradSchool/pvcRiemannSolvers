...
if __name__ == '__main__':
    ...
    x, y, z = 0, 0, 0
    lx, ly, lz = 1, 1, 1  # length of domain in each direction
    nx, ny, nz = 16, 16, 2 + 1  # number of cells in each direction
    ratio = 0.5
    ...
    for tag in (1, 2):
        gmsh.model.mesh.setTransfiniteCurve(tag, nx + 1, "Bump", ratio)
    for tag in (3, 4):
        gmsh.model.mesh.setTransfiniteCurve(tag, ny + 1, "Bump", ratio)
